...
- **Process-global mutable state** (`existingTypes`/`rootPackageOverride`) makes service-level (writeTo→clang-compile) tests order-sensitive when fixtures `#include` std headers; kept T1.4 fixtures std-free. Reset per-run if more service tests are added.

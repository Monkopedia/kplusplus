...
import com.monkopedia.krapper.generator.enumValue
...
import com.monkopedia.krapper.generator.integerType
...
import com.monkopedia.krapper.generator.numArgTypes
...
abstract class WrappedType : WrappedElement() {
    ...
    companion object :
            ...
            return when {
                ...
                referencedDecl.kind == CXCursor_EnumDecl -> {
                    ...
                    WrappedEnumType(
                        ...
                        invoke(referencedDecl.integerType, resolverBuilder),
                        constants
                    )
                }
        ...
        private fun functionPointerTypedefElement(
            ...
            val returnType = invoke(proto.result, resolverBuilder)
            val argTypes = (0 until proto.numArgTypes).map {
                invoke(proto.getArgType(it.toUInt()), resolverBuilder)
            }
        ...
        private fun referenceTypedefElement(
            ...
            val elementType = invoke(valueTypedef.typedefDeclUnderlyingType, resolverBuilder)
        ...
        private fun pointerTypedefElement(
            ...
            val elementType = invoke(elementTypedef.typedefDeclUnderlyingType, resolverBuilder)
            if (elementType == UNRESOLVABLE) return null
        ...
        private fun sizeTypedefElement(typedefDecl: CValue<CXCursor>): WrappedType? =
            when (typedefDecl.spelling.toKString()) {
                "size_type" -> invoke("size_t")
                "difference_type" -> invoke("ptrdiff_t")
                else -> null
            }
        ...
        private fun cAliasTypedefElement(typedefDecl: CValue<CXCursor>): WrappedType? {
            val name = typedefDecl.spelling.toKString()
            return if (name in C_ALIAS_TYPEDEFS) invoke(name!!) else null
        }

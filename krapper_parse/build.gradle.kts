...
//
// Publish the krapper_parse release binary to mavenLocal as an IMMUTABLE, non-SNAPSHOT
...
// (kpp.frontend.krapper_parse=seed, the gradle.properties default) — NEVER from =cpp, which
...
// untouched: nothing sets kpp.stageZeroKrapperParse, and publishing/consuming is opt-in.
val stage0Group = "com.monkopedia.kplusplus"
val stage0Artifact = "krapper_parse-stage0"
    ...
    doFirst {
        check(!stage0BuiltFromCpp) {
            "publishStage0ToMavenLocal: refusing to publish a stage-0 anchor built with " +
                "-Pkpp.frontend.krapper_parse=cpp — the anchor must be built FROM SEED (the " +
                "gradle.properties default). Drop the =cpp override and re-run."
        }
        check(krapperParseBinary.exists()) {
            "publishStage0ToMavenLocal: expected the krapper_parse release binary at " +
...
tasks.register("publishStage0ToMavenLocal") {
    group = "kplusplus"
    description =
        "Build the krapper_parse release binary FROM SEED and publish it to mavenLocal as the " +
...
// krapper_parse build can now resolve its stage-0 parser from mavenLocal instead of a hand-
// typed path, and the `resolveStage0Path` task prints the resolved File so a self-generation
// run can feed it into -Pkpp.stageZeroKrapperParse=<path> (the #122 step 1.1 override seam).
//
...
val stage0ExecFile = layout.buildDirectory.file("stage0/krapper_parse-stage0").get().asFile

tasks.register("resolveStage0Path") {
    group = "kplusplus"
    description =
        "Resolve the mavenLocal stage-0 krapper_parse binary ($stage0Group:$stage0Artifact:" +
            "$stage0Version), stage an executable copy, and print its path for " +
            "-Pkpp.stageZeroKrapperParse."
    ...
    doLast {
        ...
        // back mode 0644. runKrapperParseSync launches the parser via ProcessBuilder, which
        // needs +x — copy it out to the build dir and mark it executable.
        out.parentFile.mkdirs()
        resolved.copyTo(out, overwrite = true)
        out.setExecutable(true)
        println("STAGE0_KRAPPER_PARSE=${out.absolutePath}")
    }
}

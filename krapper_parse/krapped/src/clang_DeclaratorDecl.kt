...
import krapper.krapper_parse.internal.clang_DeclaratorDecl_align_of
import krapper.krapper_parse.internal.clang_DeclaratorDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_DeclaratorDecl_as_clang_NamedDecl
import krapper.krapper_parse.internal.clang_DeclaratorDecl_as_clang_ValueDecl
import krapper.krapper_parse.internal.clang_DeclaratorDecl_classof
import krapper.krapper_parse.internal.clang_DeclaratorDecl_classof_kind
import krapper.krapper_parse.internal.clang_DeclaratorDecl_dyncast_clang_CXXConstructorDecl
import krapper.krapper_parse.internal.clang_DeclaratorDecl_dyncast_clang_CXXDestructorDecl
import krapper.krapper_parse.internal.clang_DeclaratorDecl_dyncast_clang_CXXMethodDecl
import krapper.krapper_parse.internal.clang_DeclaratorDecl_dyncast_clang_FieldDecl
import krapper.krapper_parse.internal.clang_DeclaratorDecl_dyncast_clang_FunctionDecl
import krapper.krapper_parse.internal.clang_DeclaratorDecl_dyncast_clang_ParmVarDecl
import krapper.krapper_parse.internal.clang_DeclaratorDecl_dyncast_clang_VarDecl
...
import krapper.krapper_parse.internal.clang_DeclaratorDecl_size_of
...
// WARNING: polymorphic class with a non-virtual destructor — deleting a DeclaratorDecl through a base pointer is undefined in C++ (the derived destructor will not run). Give the base a `virtual ~...()` to fix.

@krapper.CppBinding("clang::DeclaratorDecl")
class DeclaratorDecl(
    ...
    companion object {
        val size: Int
            inline get() {
                return clang_DeclaratorDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_DeclaratorDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_DeclaratorDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_DeclaratorDecl_classof_kind(K.value)
        }
        fun MemScope.DeclaratorDecl_Holder(): DeclaratorDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return DeclaratorDecl(memory, this)
        }
    }
    inline fun asValueDecl(): ValueDecl {
        return ValueDecl(clang_DeclaratorDecl_as_clang_ValueDecl(ptr)!!, memScope)
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_DeclaratorDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_DeclaratorDecl_as_clang_Decl(ptr)!!, memScope)
    }
    inline fun asVarDecl(): VarDecl? {
        val raw: COpaquePointer = (clang_DeclaratorDecl_dyncast_clang_VarDecl(ptr) ?: return null)
        return VarDecl(raw, memScope)
    }
    inline fun asFieldDecl(): FieldDecl? {
        val raw: COpaquePointer = (clang_DeclaratorDecl_dyncast_clang_FieldDecl(ptr) ?: return null)
        return FieldDecl(raw, memScope)
    }
    inline fun asParmVarDecl(): ParmVarDecl? {
        val raw: COpaquePointer = (clang_DeclaratorDecl_dyncast_clang_ParmVarDecl(ptr) ?: return null)
        return ParmVarDecl(raw, memScope)
    }
    inline fun asFunctionDecl(): FunctionDecl? {
        val raw: COpaquePointer = (clang_DeclaratorDecl_dyncast_clang_FunctionDecl(ptr) ?: return null)
        return FunctionDecl(raw, memScope)
    }
    inline fun asCXXMethodDecl(): CXXMethodDecl? {
        val raw: COpaquePointer = (clang_DeclaratorDecl_dyncast_clang_CXXMethodDecl(ptr) ?: return null)
        return CXXMethodDecl(raw, memScope)
    }
    inline fun asCXXConstructorDecl(): CXXConstructorDecl? {
        val raw: COpaquePointer = (clang_DeclaratorDecl_dyncast_clang_CXXConstructorDecl(ptr) ?: return null)
        return CXXConstructorDecl(raw, memScope)
    }
    inline fun asCXXDestructorDecl(): CXXDestructorDecl? {
        val raw: COpaquePointer = (clang_DeclaratorDecl_dyncast_clang_CXXDestructorDecl(ptr) ?: return null)

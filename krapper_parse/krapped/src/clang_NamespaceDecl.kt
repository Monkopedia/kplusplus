...
import krapper.krapper_parse.internal.clang_NamespaceBaseDecl_get_namespace
import krapper.krapper_parse.internal.clang_NamespaceDecl_align_of
import krapper.krapper_parse.internal.clang_NamespaceDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_NamespaceDecl_as_clang_DeclContext
import krapper.krapper_parse.internal.clang_NamespaceDecl_as_clang_NamedDecl
import krapper.krapper_parse.internal.clang_NamespaceDecl_as_clang_NamespaceBaseDecl
import krapper.krapper_parse.internal.clang_NamespaceDecl_cast_from_decl_context
import krapper.krapper_parse.internal.clang_NamespaceDecl_cast_to_decl_context
import krapper.krapper_parse.internal.clang_NamespaceDecl_classof
import krapper.krapper_parse.internal.clang_NamespaceDecl_classof_kind
import krapper.krapper_parse.internal.clang_NamespaceDecl_get_anonymous_namespace
import krapper.krapper_parse.internal.clang_NamespaceDecl_get_canonical_decl
import krapper.krapper_parse.internal.clang_NamespaceDecl_is_anonymous_namespace
import krapper.krapper_parse.internal.clang_NamespaceDecl_is_inline
import krapper.krapper_parse.internal.clang_NamespaceDecl_is_nested
import krapper.krapper_parse.internal.clang_NamespaceDecl_set_anonymous_namespace
import krapper.krapper_parse.internal.clang_NamespaceDecl_set_inline
import krapper.krapper_parse.internal.clang_NamespaceDecl_set_nested
import krapper.krapper_parse.internal.clang_NamespaceDecl_size_of
...
@krapper.CppBinding("clang::NamespaceDecl")
class NamespaceDecl(
    ...
    inline fun isAnonymousNamespace(): Boolean {
        return clang_NamespaceDecl_is_anonymous_namespace(ptr)
    }
    inline fun isInline(): Boolean {
        return clang_NamespaceDecl_is_inline(ptr)
    }
    inline fun setInline(Inline: Boolean): Unit {
        return clang_NamespaceDecl_set_inline(ptr, Inline)
    }
    inline fun isNested(): Boolean {
        return clang_NamespaceDecl_is_nested(ptr)
    }
    inline fun setNested(Nested: Boolean): Unit {
        return clang_NamespaceDecl_set_nested(ptr, Nested)
    }
    inline fun getAnonymousNamespace(): NamespaceDecl? {
        return NamespaceDecl((clang_NamespaceDecl_get_anonymous_namespace(ptr) ?: return null), memScope)
    }
    inline fun setAnonymousNamespace(D: NamespaceDecl?): Unit {
        return clang_NamespaceDecl_set_anonymous_namespace(ptr, D?.ptr)
    }
    inline fun namespaceDeclGetCanonicalDecl(): NamespaceDecl? {
        return NamespaceDecl((clang_NamespaceDecl_get_canonical_decl(ptr) ?: return null), memScope)
    }
    companion object {
        val size: Int
            inline get() {
                return clang_NamespaceDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_NamespaceDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_NamespaceDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_NamespaceDecl_classof_kind(K.value)
        }
        inline fun MemScope.castToDeclContext(D: NamespaceDecl?): DeclContext? {
            return DeclContext((clang_NamespaceDecl_cast_to_decl_context(D?.ptr) ?: return null), memScope)
        }
        inline fun MemScope.castFromDeclContext(DC: DeclContext?): NamespaceDecl? {
            return NamespaceDecl((clang_NamespaceDecl_cast_from_decl_context(DC?.ptr) ?: return null), memScope)
        }
        fun MemScope.NamespaceDecl_Holder(): NamespaceDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return NamespaceDecl(memory, this)
        }
    }
    inline fun asNamespaceBaseDecl(): NamespaceBaseDecl {
        return NamespaceBaseDecl(clang_NamespaceDecl_as_clang_NamespaceBaseDecl(ptr)!!, memScope)
    }
    inline fun asDeclContext(): DeclContext {
        return DeclContext(clang_NamespaceDecl_as_clang_DeclContext(ptr)!!, memScope)
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_NamespaceDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_NamespaceDecl_as_clang_Decl(ptr)!!, memScope)
    }
}

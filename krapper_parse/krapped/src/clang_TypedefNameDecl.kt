...
import krapper.krapper_parse.internal.clang_TypedefNameDecl_align_of
import krapper.krapper_parse.internal.clang_TypedefNameDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_TypedefNameDecl_as_clang_NamedDecl
import krapper.krapper_parse.internal.clang_TypedefNameDecl_as_clang_TypeDecl
import krapper.krapper_parse.internal.clang_TypedefNameDecl_classof
import krapper.krapper_parse.internal.clang_TypedefNameDecl_classof_kind
import krapper.krapper_parse.internal.clang_TypedefNameDecl_get_anon_decl_with_typedef_name
import krapper.krapper_parse.internal.clang_TypedefNameDecl_get_canonical_decl
import krapper.krapper_parse.internal.clang_TypedefNameDecl_get_underlying_type
import krapper.krapper_parse.internal.clang_TypedefNameDecl_is_moded
import krapper.krapper_parse.internal.clang_TypedefNameDecl_is_transparent_tag
import krapper.krapper_parse.internal.clang_TypedefNameDecl_size_of
...
@krapper.CppBinding("clang::TypedefNameDecl")
class TypedefNameDecl(
    ...
    inline fun isModed(): Boolean {
        return clang_TypedefNameDecl_is_moded(ptr)
    }
    inline fun getUnderlyingType(): QualType {
        val retValue: QualType = memScope.QualType_Holder()
        clang_TypedefNameDecl_get_underlying_type(ptr, retValue.ptr)
        return retValue
    }
    inline fun typedefNameDeclGetCanonicalDecl(): TypedefNameDecl? {
        return TypedefNameDecl((clang_TypedefNameDecl_get_canonical_decl(ptr) ?: return null), memScope)
    }
    inline fun getAnonDeclWithTypedefName(AnyRedecl: Boolean = false): TagDeclApi? {
        return TagDecl((clang_TypedefNameDecl_get_anon_decl_with_typedef_name(ptr, AnyRedecl) ?: return null), memScope)
    }
    inline fun isTransparentTag(): Boolean {
        return clang_TypedefNameDecl_is_transparent_tag(ptr)
    }
    companion object {
        val size: Int
            inline get() {
                return clang_TypedefNameDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_TypedefNameDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_TypedefNameDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_TypedefNameDecl_classof_kind(K.value)
        }
        fun MemScope.TypedefNameDecl_Holder(): TypedefNameDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return TypedefNameDecl(memory, this)
        }
    }
    inline fun asTypeDecl(): TypeDecl {
        return TypeDecl(clang_TypedefNameDecl_as_clang_TypeDecl(ptr)!!, memScope)
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_TypedefNameDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_TypedefNameDecl_as_clang_Decl(ptr)!!, memScope)
    }
}

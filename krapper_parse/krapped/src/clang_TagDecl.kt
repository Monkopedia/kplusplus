...
import krapper.krapper_parse.internal.clang_TagDecl_align_of
import krapper.krapper_parse.internal.clang_TagDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_TagDecl_as_clang_DeclContext
import krapper.krapper_parse.internal.clang_TagDecl_as_clang_NamedDecl
import krapper.krapper_parse.internal.clang_TagDecl_as_clang_TypeDecl
import krapper.krapper_parse.internal.clang_TagDecl_cast_from_decl_context
import krapper.krapper_parse.internal.clang_TagDecl_cast_to_decl_context
import krapper.krapper_parse.internal.clang_TagDecl_classof
import krapper.krapper_parse.internal.clang_TagDecl_classof_kind
import krapper.krapper_parse.internal.clang_TagDecl_demote_this_definition_to_declaration
import krapper.krapper_parse.internal.clang_TagDecl_dyncast_clang_CXXRecordDecl
import krapper.krapper_parse.internal.clang_TagDecl_dyncast_clang_ClassTemplateSpecializationDecl
import krapper.krapper_parse.internal.clang_TagDecl_dyncast_clang_EnumDecl
import krapper.krapper_parse.internal.clang_TagDecl_dyncast_clang_RecordDecl
...
import krapper.krapper_parse.internal.clang_TagDecl_size_of
...
// WARNING: polymorphic class with a non-virtual destructor — deleting a TagDecl through a base pointer is undefined in C++ (the derived destructor will not run). Give the base a `virtual ~...()` to fix.

@krapper.CppBinding("clang::TagDecl")
class TagDecl(
    ...
    companion object {
        val size: Int
            inline get() {
                return clang_TagDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_TagDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_TagDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_TagDecl_classof_kind(K.value)
        }
        inline fun MemScope.castToDeclContext(D: TagDeclApi?): DeclContext? {
            return DeclContext((clang_TagDecl_cast_to_decl_context(D?.ptr) ?: return null), memScope)
        }
        inline fun MemScope.castFromDeclContext(DC: DeclContext?): TagDeclApi? {
            return TagDecl((clang_TagDecl_cast_from_decl_context(DC?.ptr) ?: return null), memScope)
        }
        fun MemScope.TagDecl_Holder(): TagDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return TagDecl(memory, this)
        }
    }
    inline fun asTypeDecl(): TypeDecl {
        return TypeDecl(clang_TagDecl_as_clang_TypeDecl(ptr)!!, memScope)
    }
    inline fun asDeclContext(): DeclContext {
        return DeclContext(clang_TagDecl_as_clang_DeclContext(ptr)!!, memScope)
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_TagDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_TagDecl_as_clang_Decl(ptr)!!, memScope)
    }
    inline fun asEnumDecl(): EnumDecl? {
        val raw: COpaquePointer = (clang_TagDecl_dyncast_clang_EnumDecl(ptr) ?: return null)
        return EnumDecl(raw, memScope)
    }
    inline fun asRecordDecl(): RecordDecl? {
        val raw: COpaquePointer = (clang_TagDecl_dyncast_clang_RecordDecl(ptr) ?: return null)
        return RecordDecl(raw, memScope)
    }
    inline fun asClassTemplateSpecializationDecl(): ClassTemplateSpecializationDecl? {
        val raw: COpaquePointer = (clang_TagDecl_dyncast_clang_ClassTemplateSpecializationDecl(ptr) ?: return null)
        return ClassTemplateSpecializationDecl(raw, memScope)
    }
    inline fun asCXXRecordDecl(): CXXRecordDecl? {
        val raw: COpaquePointer = (clang_TagDecl_dyncast_clang_CXXRecordDecl(ptr) ?: return null)

...
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_align_of
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_as_clang_NamedDecl
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_as_clang_TypeDecl
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_classof
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_classof_kind
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_default_argument_was_inherited
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_get_depth
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_get_index
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_has_default_argument
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_has_type_constraint
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_is_pack_expansion
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_is_parameter_pack
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_remove_default_argument
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_set_declared_with_typename
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_set_inherited_default_argument
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_size_of
import krapper.krapper_parse.internal.clang_TemplateTypeParmDecl_was_declared_with_typename
...
@krapper.CppBinding("clang::TemplateTypeParmDecl")
class TemplateTypeParmDecl(
    override val ptr: COpaquePointer,
    val memScope: MemScope,
) : clang.TypeDeclApi, clang.NamedDeclApi, clang.DeclApi {
    ...
    inline fun wasDeclaredWithTypename(): Boolean {
        return clang_TemplateTypeParmDecl_was_declared_with_typename(ptr)
    }
    inline fun hasDefaultArgument(): Boolean {
        return clang_TemplateTypeParmDecl_has_default_argument(ptr)
    }
    inline fun defaultArgumentWasInherited(): Boolean {
        return clang_TemplateTypeParmDecl_default_argument_was_inherited(ptr)
    }
    inline fun setInheritedDefaultArgument(C: ASTContext, Prev: TemplateTypeParmDecl?): Unit {
        return clang_TemplateTypeParmDecl_set_inherited_default_argument(ptr, C.ptr, Prev?.ptr)
    }
    inline fun removeDefaultArgument(): Unit {
        return clang_TemplateTypeParmDecl_remove_default_argument(ptr)
    }
    inline fun setDeclaredWithTypename(withTypename: Boolean): Unit {
        return clang_TemplateTypeParmDecl_set_declared_with_typename(ptr, withTypename)
    }
    inline fun getDepth(): UInt {
        return clang_TemplateTypeParmDecl_get_depth(ptr)
    }
    inline fun getIndex(): UInt {
        return clang_TemplateTypeParmDecl_get_index(ptr)
    }
    inline fun templateTypeParmDeclIsParameterPack(): Boolean {
        return clang_TemplateTypeParmDecl_is_parameter_pack(ptr)
    }
    inline fun isPackExpansion(): Boolean {
        return clang_TemplateTypeParmDecl_is_pack_expansion(ptr)
    }
    inline fun hasTypeConstraint(): Boolean {
        return clang_TemplateTypeParmDecl_has_type_constraint(ptr)
    }
    companion object {
        val size: Int
            inline get() {
                return clang_TemplateTypeParmDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_TemplateTypeParmDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_TemplateTypeParmDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_TemplateTypeParmDecl_classof_kind(K.value)
        }
        fun MemScope.TemplateTypeParmDecl_Holder(): TemplateTypeParmDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return TemplateTypeParmDecl(memory, this)
        }
    }
    inline fun asTypeDecl(): TypeDecl {
        return TypeDecl(clang_TemplateTypeParmDecl_as_clang_TypeDecl(ptr)!!, memScope)
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_TemplateTypeParmDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_TemplateTypeParmDecl_as_clang_Decl(ptr)!!, memScope)
    }
}

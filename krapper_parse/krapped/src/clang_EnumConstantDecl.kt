...
import krapper.krapper_parse.internal.clang_EnumConstantDecl_align_of
import krapper.krapper_parse.internal.clang_EnumConstantDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_EnumConstantDecl_as_clang_NamedDecl
import krapper.krapper_parse.internal.clang_EnumConstantDecl_as_clang_ValueDecl
import krapper.krapper_parse.internal.clang_EnumConstantDecl_classof
import krapper.krapper_parse.internal.clang_EnumConstantDecl_classof_kind
import krapper.krapper_parse.internal.clang_EnumConstantDecl_get_canonical_decl
import krapper.krapper_parse.internal.clang_EnumConstantDecl_get_init_val
import krapper.krapper_parse.internal.clang_EnumConstantDecl_set_init_val
import krapper.krapper_parse.internal.clang_EnumConstantDecl_size_of
...
@krapper.CppBinding("clang::EnumConstantDecl")
class EnumConstantDecl(
    ...
    inline fun getInitVal(): APSInt {
        val retValue: APSInt = memScope.APSInt_Holder()
        clang_EnumConstantDecl_get_init_val(ptr, retValue.ptr)
        return retValue
    }
    inline fun setInitVal(C: ASTContext, V: APSInt): Unit {
        return clang_EnumConstantDecl_set_init_val(ptr, C.ptr, V.ptr)
    }
    inline fun enumConstantDeclGetCanonicalDecl(): EnumConstantDecl? {
        return EnumConstantDecl((clang_EnumConstantDecl_get_canonical_decl(ptr) ?: return null), memScope)
    }
    companion object {
        val size: Int
            inline get() {
                return clang_EnumConstantDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_EnumConstantDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_EnumConstantDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_EnumConstantDecl_classof_kind(K.value)
        }
        fun MemScope.EnumConstantDecl_Holder(): EnumConstantDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return EnumConstantDecl(memory, this)
        }
    }
    inline fun asValueDecl(): ValueDecl {
        return ValueDecl(clang_EnumConstantDecl_as_clang_ValueDecl(ptr)!!, memScope)
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_EnumConstantDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_EnumConstantDecl_as_clang_Decl(ptr)!!, memScope)
    }
}

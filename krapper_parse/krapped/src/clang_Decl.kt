...
import clang.AccessSpecifier.Companion.fromValue as CompanionfromValue
...
import clang.langOptionsBase.StrictFlexArraysLevelKind
...
import krapper.krapper_parse.internal.clang_Decl_add
import krapper.krapper_parse.internal.clang_Decl_align_of
import krapper.krapper_parse.internal.clang_Decl_can_be_weak_imported
import krapper.krapper_parse.internal.clang_Decl_cast_from_decl_context
import krapper.krapper_parse.internal.clang_Decl_cast_to_decl_context
import krapper.krapper_parse.internal.clang_Decl_classof_kind
...
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_CXXConstructorDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_CXXDestructorDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_CXXMethodDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_CXXRecordDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_ClassTemplateSpecializationDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_DeclaratorDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_EnumConstantDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_EnumDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_FieldDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_FunctionDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_NamedDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_NamespaceBaseDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_NamespaceDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_ParmVarDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_RecordDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_TagDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_TemplateDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_TemplateTypeParmDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_TranslationUnitDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_TypeDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_TypedefNameDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_ValueDecl
import krapper.krapper_parse.internal.clang_Decl_dyncast_clang_VarDecl
import krapper.krapper_parse.internal.clang_Decl_enable_statistics
...
import krapper.krapper_parse.internal.clang_Decl_get_identifier_namespace_for_kind
...
import krapper.krapper_parse.internal.clang_Decl_get_most_recent_decl
...
import krapper.krapper_parse.internal.clang_Decl_is_flexible_array_member_like
...
import krapper.krapper_parse.internal.clang_Decl_is_tag_identifier_namespace
...
import krapper.krapper_parse.internal.clang_Decl_print_stats
...
import krapper.krapper_parse.internal.clang_Decl_size_of
import std.Vector__Decl_P
import std.Vector__Decl_P.Companion.Vector__Decl_P_Holder

// BEGIN KRAPPER GEN for clang::Decl
// WARNING: polymorphic class with a non-virtual destructor — deleting a Decl through a base pointer is undefined in C++ (the derived destructor will not run). Give the base a `virtual ~...()` to fix.

@krapper.CppBinding("clang::Decl")
class Decl(
    override val ptr: COpaquePointer,
    val memScope: MemScope,
) : clang.DeclApi {
    inline fun getKind(): Kind {
        return fromValue(clang_Decl_get_kind(ptr))
    }
    ...
    inline fun getAccess(): AccessSpecifier {
        return CompanionfromValue(clang_Decl_get_access(ptr))
    }
    inline fun getAccessUnsafe(): AccessSpecifier {
        return CompanionfromValue(clang_Decl_get_access_unsafe(ptr))
    }
    ...
    inline fun getMostRecentDecl(): DeclApi? {
        return Decl((clang_Decl_get_most_recent_decl(ptr) ?: return null), memScope)
    }
    ...
    companion object {
        val size: Int
            inline get() {
                return clang_Decl_size_of()
            }

        val align: Int
            inline get() {
                return clang_Decl_align_of()
            }

        inline fun MemScope.isFlexibleArrayMemberLike(Context: ASTContext, D: DeclApi?, Ty: QualType, StrictFlexArraysLevel: StrictFlexArraysLevelKind, IgnoreTemplateOrMacroSubstitution: Boolean): Boolean {
            return clang_Decl_is_flexible_array_member_like(Context.ptr, D?.ptr, Ty.ptr, StrictFlexArraysLevel.value, IgnoreTemplateOrMacroSubstitution)
        }
        inline fun MemScope.getIdentifierNamespaceForKind(DK: Kind): UInt {
            return clang_Decl_get_identifier_namespace_for_kind(DK.value)
        }
        inline fun MemScope.isTagIdentifierNamespace(NS: UInt): Boolean {
            return clang_Decl_is_tag_identifier_namespace(NS)
        }
        inline fun MemScope.add(k: Kind): Unit {
            return clang_Decl_add(k.value)
        }
        inline fun MemScope.EnableStatistics(): Unit {
            return clang_Decl_enable_statistics()
        }
        inline fun MemScope.PrintStats(): Unit {
            return clang_Decl_print_stats()
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_Decl_classof_kind(K.value)
        }
        inline fun MemScope.castToDeclContext(_arg_0: DeclApi?): DeclContext? {
            return DeclContext((clang_Decl_cast_to_decl_context(_arg_0?.ptr) ?: return null), memScope)
        }
        inline fun MemScope.castFromDeclContext(_arg_0: DeclContext?): DeclApi? {
            return Decl((clang_Decl_cast_from_decl_context(_arg_0?.ptr) ?: return null), memScope)
        }
        fun MemScope.Decl_Holder(): Decl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return Decl(memory, this)
        }
    }
    inline fun asNamedDecl(): NamedDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_NamedDecl(ptr) ?: return null)
        return NamedDecl(raw, memScope)
    }
    inline fun asDeclaratorDecl(): DeclaratorDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_DeclaratorDecl(ptr) ?: return null)
        return DeclaratorDecl(raw, memScope)
    }
    inline fun asVarDecl(): VarDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_VarDecl(ptr) ?: return null)
        return VarDecl(raw, memScope)
    }
    inline fun asValueDecl(): ValueDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_ValueDecl(ptr) ?: return null)
        return ValueDecl(raw, memScope)
    }
    inline fun asEnumConstantDecl(): EnumConstantDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_EnumConstantDecl(ptr) ?: return null)
        return EnumConstantDecl(raw, memScope)
    }
    inline fun asEnumDecl(): EnumDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_EnumDecl(ptr) ?: return null)
        return EnumDecl(raw, memScope)
    }
    inline fun asTypeDecl(): TypeDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_TypeDecl(ptr) ?: return null)
        return TypeDecl(raw, memScope)
    }
    inline fun asTypedefNameDecl(): TypedefNameDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_TypedefNameDecl(ptr) ?: return null)
        return TypedefNameDecl(raw, memScope)
    }
    inline fun asTagDecl(): TagDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_TagDecl(ptr) ?: return null)
        return TagDecl(raw, memScope)
    }
    inline fun asFieldDecl(): FieldDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_FieldDecl(ptr) ?: return null)
        return FieldDecl(raw, memScope)
    }
    inline fun asRecordDecl(): RecordDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_RecordDecl(ptr) ?: return null)
        return RecordDecl(raw, memScope)
    }
    inline fun asParmVarDecl(): ParmVarDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_ParmVarDecl(ptr) ?: return null)
        return ParmVarDecl(raw, memScope)
    }
    inline fun asFunctionDecl(): FunctionDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_FunctionDecl(ptr) ?: return null)
        return FunctionDecl(raw, memScope)
    }
    inline fun asCXXMethodDecl(): CXXMethodDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_CXXMethodDecl(ptr) ?: return null)
        return CXXMethodDecl(raw, memScope)
    }
    inline fun asCXXConstructorDecl(): CXXConstructorDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_CXXConstructorDecl(ptr) ?: return null)
        return CXXConstructorDecl(raw, memScope)
    }
    inline fun asCXXDestructorDecl(): CXXDestructorDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_CXXDestructorDecl(ptr) ?: return null)
        return CXXDestructorDecl(raw, memScope)
    }
    inline fun asClassTemplateSpecializationDecl(): ClassTemplateSpecializationDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_ClassTemplateSpecializationDecl(ptr) ?: return null)
        return ClassTemplateSpecializationDecl(raw, memScope)
    }
    inline fun asCXXRecordDecl(): CXXRecordDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_CXXRecordDecl(ptr) ?: return null)
        return CXXRecordDecl(raw, memScope)
    }
    inline fun asNamespaceBaseDecl(): NamespaceBaseDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_NamespaceBaseDecl(ptr) ?: return null)
        return NamespaceBaseDecl(raw, memScope)
    }
    inline fun asNamespaceDecl(): NamespaceDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_NamespaceDecl(ptr) ?: return null)
        return NamespaceDecl(raw, memScope)
    }
    inline fun asTranslationUnitDecl(): TranslationUnitDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_TranslationUnitDecl(ptr) ?: return null)
        return TranslationUnitDecl(raw, memScope)
    }
    inline fun asTemplateTypeParmDecl(): TemplateTypeParmDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_TemplateTypeParmDecl(ptr) ?: return null)
        return TemplateTypeParmDecl(raw, memScope)
    }
    inline fun asTemplateDecl(): TemplateDecl? {
        val raw: COpaquePointer = (clang_Decl_dyncast_clang_TemplateDecl(ptr) ?: return null)
        return TemplateDecl(raw, memScope)
    }
}

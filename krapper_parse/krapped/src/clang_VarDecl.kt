...
import clang.ThreadStorageClassSpecifier.Companion.fromValue as ThreadStorageClassSpecifierCompanionfromValue
...
import krapper.krapper_parse.internal.clang_Decl_get_canonical_decl
...
import krapper.krapper_parse.internal.clang_VarDecl_align_of
import krapper.krapper_parse.internal.clang_VarDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_VarDecl_as_clang_DeclaratorDecl
import krapper.krapper_parse.internal.clang_VarDecl_as_clang_NamedDecl
import krapper.krapper_parse.internal.clang_VarDecl_as_clang_ValueDecl
import krapper.krapper_parse.internal.clang_VarDecl_classof
import krapper.krapper_parse.internal.clang_VarDecl_classof_kind
import krapper.krapper_parse.internal.clang_VarDecl_demote_this_definition_to_declaration
import krapper.krapper_parse.internal.clang_VarDecl_dyncast_clang_ParmVarDecl
...
import krapper.krapper_parse.internal.clang_VarDecl_get_storage_class_specifier_string
...
import krapper.krapper_parse.internal.clang_VarDecl_size_of
...
// WARNING: polymorphic class with a non-virtual destructor — deleting a VarDecl through a base pointer is undefined in C++ (the derived destructor will not run). Give the base a `virtual ~...()` to fix.

@krapper.CppBinding("clang::VarDecl")
class VarDecl(
    ...
    inline fun getStorageClass(): StorageClass {
        return StorageClassCompanionfromValue(clang_VarDecl_get_storage_class(ptr))
    }
    ...
    inline fun getTSCSpec(): ThreadStorageClassSpecifier {
        return ThreadStorageClassSpecifierCompanionfromValue(clang_VarDecl_get_tsc_spec(ptr))
    }
    ...
    companion object {
        val size: Int
            inline get() {
                return clang_VarDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_VarDecl_align_of()
            }

        inline fun MemScope.getStorageClassSpecifierString(SC: StorageClass): String? {
            val str: CPointer<ByteVar>? = clang_VarDecl_get_storage_class_specifier_string(SC.value)
            val ret: String? = str?.toKString()
            return ret
        }
        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_VarDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_VarDecl_classof_kind(K.value)
        }
        fun MemScope.VarDecl_Holder(): VarDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return VarDecl(memory, this)
        }
    }
    inline fun asDeclaratorDecl(): DeclaratorDecl {
        return DeclaratorDecl(clang_VarDecl_as_clang_DeclaratorDecl(ptr)!!, memScope)
    }
    inline fun asValueDecl(): ValueDecl {
        return ValueDecl(clang_VarDecl_as_clang_ValueDecl(ptr)!!, memScope)
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_VarDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_VarDecl_as_clang_Decl(ptr)!!, memScope)
    }
    inline fun asParmVarDecl(): ParmVarDecl? {
        val raw: COpaquePointer = (clang_VarDecl_dyncast_clang_ParmVarDecl(ptr) ?: return null)
        return ParmVarDecl(raw, memScope)
    }
}

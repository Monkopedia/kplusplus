...
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_align_of
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_as_clang_DeclContext
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_cast_from_decl_context
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_cast_to_decl_context
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_classof
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_classof_kind
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_create
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_get_anonymous_namespace
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_get_ast_context
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_get_canonical_decl
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_set_anonymous_namespace
import krapper.krapper_parse.internal.clang_TranslationUnitDecl_size_of
...
@krapper.CppBinding("clang::TranslationUnitDecl")
class TranslationUnitDecl(
    ...
    inline fun translationUnitDeclGetASTContext(): ASTContext? {
        return ASTContext((clang_TranslationUnitDecl_get_ast_context(ptr) ?: return null), memScope)
    }
    inline fun getAnonymousNamespace(): NamespaceDecl? {
        return NamespaceDecl((clang_TranslationUnitDecl_get_anonymous_namespace(ptr) ?: return null), memScope)
    }
    inline fun setAnonymousNamespace(D: NamespaceDecl?): Unit {
        return clang_TranslationUnitDecl_set_anonymous_namespace(ptr, D?.ptr)
    }
    inline fun translationUnitDeclGetCanonicalDecl(): TranslationUnitDecl? {
        return TranslationUnitDecl((clang_TranslationUnitDecl_get_canonical_decl(ptr) ?: return null), memScope)
    }
    companion object {
        val size: Int
            inline get() {
                return clang_TranslationUnitDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_TranslationUnitDecl_align_of()
            }

        inline fun MemScope.Create(C: ASTContext): TranslationUnitDecl? {
            return TranslationUnitDecl((clang_TranslationUnitDecl_create(C.ptr) ?: return null), memScope)
        }
        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_TranslationUnitDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_TranslationUnitDecl_classof_kind(K.value)
        }
        inline fun MemScope.castToDeclContext(D: TranslationUnitDecl?): DeclContext? {
            return DeclContext((clang_TranslationUnitDecl_cast_to_decl_context(D?.ptr) ?: return null), memScope)
        }
        inline fun MemScope.castFromDeclContext(DC: DeclContext?): TranslationUnitDecl? {
            return TranslationUnitDecl((clang_TranslationUnitDecl_cast_from_decl_context(DC?.ptr) ?: return null), memScope)
        }
        fun MemScope.TranslationUnitDecl_Holder(): TranslationUnitDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return TranslationUnitDecl(memory, this)
        }
    }
    inline fun asDecl(): Decl {
        return Decl(clang_TranslationUnitDecl_as_clang_Decl(ptr)!!, memScope)
    }
    inline fun asDeclContext(): DeclContext {
        return DeclContext(clang_TranslationUnitDecl_as_clang_DeclContext(ptr)!!, memScope)
    }
}

...
import krapper.krapper_parse.internal.clang_NamedDecl_align_of
import krapper.krapper_parse.internal.clang_NamedDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_NamedDecl_classof
import krapper.krapper_parse.internal.clang_NamedDecl_classof_kind
import krapper.krapper_parse.internal.clang_NamedDecl_declaration_replaces
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_CXXConstructorDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_CXXDestructorDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_CXXMethodDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_CXXRecordDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_ClassTemplateSpecializationDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_DeclaratorDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_EnumConstantDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_EnumDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_FieldDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_FunctionDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_NamespaceBaseDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_NamespaceDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_ParmVarDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_RecordDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_TagDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_TemplateDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_TemplateTypeParmDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_TypeDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_TypedefNameDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_ValueDecl
import krapper.krapper_parse.internal.clang_NamedDecl_dyncast_clang_VarDecl
...
import krapper.krapper_parse.internal.clang_NamedDecl_size_of
...
// WARNING: polymorphic class with a non-virtual destructor — deleting a NamedDecl through a base pointer is undefined in C++ (the derived destructor will not run). Give the base a `virtual ~...()` to fix.

@krapper.CppBinding("clang::NamedDecl")
class NamedDecl(
    override val ptr: COpaquePointer,
    val memScope: MemScope,
) : clang.NamedDeclApi, clang.DeclApi {
    ...
    companion object {
        val size: Int
            inline get() {
                return clang_NamedDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_NamedDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_NamedDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_NamedDecl_classof_kind(K.value)
        }
        fun MemScope.NamedDecl_Holder(): NamedDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return NamedDecl(memory, this)
        }
    }
    inline fun asDecl(): Decl {
        return Decl(clang_NamedDecl_as_clang_Decl(ptr)!!, memScope)
    }
    inline fun asDeclaratorDecl(): DeclaratorDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_DeclaratorDecl(ptr) ?: return null)
        return DeclaratorDecl(raw, memScope)
    }
    inline fun asVarDecl(): VarDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_VarDecl(ptr) ?: return null)
        return VarDecl(raw, memScope)
    }
    inline fun asValueDecl(): ValueDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_ValueDecl(ptr) ?: return null)
        return ValueDecl(raw, memScope)
    }
    inline fun asEnumConstantDecl(): EnumConstantDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_EnumConstantDecl(ptr) ?: return null)
        return EnumConstantDecl(raw, memScope)
    }
    inline fun asEnumDecl(): EnumDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_EnumDecl(ptr) ?: return null)
        return EnumDecl(raw, memScope)
    }
    inline fun asTypeDecl(): TypeDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_TypeDecl(ptr) ?: return null)
        return TypeDecl(raw, memScope)
    }
    inline fun asTypedefNameDecl(): TypedefNameDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_TypedefNameDecl(ptr) ?: return null)
        return TypedefNameDecl(raw, memScope)
    }
    inline fun asTagDecl(): TagDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_TagDecl(ptr) ?: return null)
        return TagDecl(raw, memScope)
    }
    inline fun asFieldDecl(): FieldDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_FieldDecl(ptr) ?: return null)
        return FieldDecl(raw, memScope)
    }
    inline fun asRecordDecl(): RecordDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_RecordDecl(ptr) ?: return null)
        return RecordDecl(raw, memScope)
    }
    inline fun asParmVarDecl(): ParmVarDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_ParmVarDecl(ptr) ?: return null)
        return ParmVarDecl(raw, memScope)
    }
    inline fun asFunctionDecl(): FunctionDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_FunctionDecl(ptr) ?: return null)
        return FunctionDecl(raw, memScope)
    }
    inline fun asCXXMethodDecl(): CXXMethodDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_CXXMethodDecl(ptr) ?: return null)
        return CXXMethodDecl(raw, memScope)
    }
    inline fun asCXXConstructorDecl(): CXXConstructorDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_CXXConstructorDecl(ptr) ?: return null)
        return CXXConstructorDecl(raw, memScope)
    }
    inline fun asCXXDestructorDecl(): CXXDestructorDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_CXXDestructorDecl(ptr) ?: return null)
        return CXXDestructorDecl(raw, memScope)
    }
    inline fun asClassTemplateSpecializationDecl(): ClassTemplateSpecializationDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_ClassTemplateSpecializationDecl(ptr) ?: return null)
        return ClassTemplateSpecializationDecl(raw, memScope)
    }
    inline fun asCXXRecordDecl(): CXXRecordDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_CXXRecordDecl(ptr) ?: return null)
        return CXXRecordDecl(raw, memScope)
    }
    inline fun asNamespaceBaseDecl(): NamespaceBaseDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_NamespaceBaseDecl(ptr) ?: return null)
        return NamespaceBaseDecl(raw, memScope)
    }
    inline fun asNamespaceDecl(): NamespaceDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_NamespaceDecl(ptr) ?: return null)
        return NamespaceDecl(raw, memScope)
    }
    inline fun asTemplateTypeParmDecl(): TemplateTypeParmDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_TemplateTypeParmDecl(ptr) ?: return null)
        return TemplateTypeParmDecl(raw, memScope)
    }
    inline fun asTemplateDecl(): TemplateDecl? {
        val raw: COpaquePointer = (clang_NamedDecl_dyncast_clang_TemplateDecl(ptr) ?: return null)

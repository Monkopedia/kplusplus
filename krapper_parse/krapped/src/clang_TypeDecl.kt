...
import krapper.krapper_parse.internal.clang_TypeDecl_align_of
import krapper.krapper_parse.internal.clang_TypeDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_TypeDecl_as_clang_NamedDecl
import krapper.krapper_parse.internal.clang_TypeDecl_classof
import krapper.krapper_parse.internal.clang_TypeDecl_classof_kind
import krapper.krapper_parse.internal.clang_TypeDecl_dyncast_clang_CXXRecordDecl
import krapper.krapper_parse.internal.clang_TypeDecl_dyncast_clang_ClassTemplateSpecializationDecl
import krapper.krapper_parse.internal.clang_TypeDecl_dyncast_clang_EnumDecl
import krapper.krapper_parse.internal.clang_TypeDecl_dyncast_clang_RecordDecl
import krapper.krapper_parse.internal.clang_TypeDecl_dyncast_clang_TagDecl
import krapper.krapper_parse.internal.clang_TypeDecl_dyncast_clang_TemplateTypeParmDecl
import krapper.krapper_parse.internal.clang_TypeDecl_dyncast_clang_TypedefNameDecl
...
import krapper.krapper_parse.internal.clang_TypeDecl_size_of
...
// WARNING: polymorphic class with a non-virtual destructor — deleting a TypeDecl through a base pointer is undefined in C++ (the derived destructor will not run). Give the base a `virtual ~...()` to fix.

@krapper.CppBinding("clang::TypeDecl")
class TypeDecl(
    ...
    companion object {
        val size: Int
            inline get() {
                return clang_TypeDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_TypeDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_TypeDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_TypeDecl_classof_kind(K.value)
        }
        fun MemScope.TypeDecl_Holder(): TypeDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return TypeDecl(memory, this)
        }
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_TypeDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_TypeDecl_as_clang_Decl(ptr)!!, memScope)
    }
    inline fun asEnumDecl(): EnumDecl? {
        val raw: COpaquePointer = (clang_TypeDecl_dyncast_clang_EnumDecl(ptr) ?: return null)
        return EnumDecl(raw, memScope)
    }
    inline fun asTypedefNameDecl(): TypedefNameDecl? {
        val raw: COpaquePointer = (clang_TypeDecl_dyncast_clang_TypedefNameDecl(ptr) ?: return null)
        return TypedefNameDecl(raw, memScope)
    }
    inline fun asTagDecl(): TagDecl? {
        val raw: COpaquePointer = (clang_TypeDecl_dyncast_clang_TagDecl(ptr) ?: return null)
        return TagDecl(raw, memScope)
    }
    inline fun asRecordDecl(): RecordDecl? {
        val raw: COpaquePointer = (clang_TypeDecl_dyncast_clang_RecordDecl(ptr) ?: return null)
        return RecordDecl(raw, memScope)
    }
    inline fun asClassTemplateSpecializationDecl(): ClassTemplateSpecializationDecl? {
        val raw: COpaquePointer = (clang_TypeDecl_dyncast_clang_ClassTemplateSpecializationDecl(ptr) ?: return null)
        return ClassTemplateSpecializationDecl(raw, memScope)
    }
    inline fun asCXXRecordDecl(): CXXRecordDecl? {
        val raw: COpaquePointer = (clang_TypeDecl_dyncast_clang_CXXRecordDecl(ptr) ?: return null)
        return CXXRecordDecl(raw, memScope)
    }
    inline fun asTemplateTypeParmDecl(): TemplateTypeParmDecl? {
        val raw: COpaquePointer = (clang_TypeDecl_dyncast_clang_TemplateTypeParmDecl(ptr) ?: return null)

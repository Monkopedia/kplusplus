...
import krapper.krapper_parse.internal.clang_TemplateDecl_align_of
import krapper.krapper_parse.internal.clang_TemplateDecl_as_clang_Decl
import krapper.krapper_parse.internal.clang_TemplateDecl_as_clang_NamedDecl
import krapper.krapper_parse.internal.clang_TemplateDecl_classof
import krapper.krapper_parse.internal.clang_TemplateDecl_classof_kind
import krapper.krapper_parse.internal.clang_TemplateDecl_get_template_parameters
import krapper.krapper_parse.internal.clang_TemplateDecl_get_templated_decl
import krapper.krapper_parse.internal.clang_TemplateDecl_has_associated_constraints
import krapper.krapper_parse.internal.clang_TemplateDecl_init
import krapper.krapper_parse.internal.clang_TemplateDecl_is_type_alias
import krapper.krapper_parse.internal.clang_TemplateDecl_set_template_parameters
import krapper.krapper_parse.internal.clang_TemplateDecl_size_of
...
@krapper.CppBinding("clang::TemplateDecl")
class TemplateDecl(
    ...
    inline fun getTemplateParameters(): TemplateParameterList? {
        return TemplateParameterList((clang_TemplateDecl_get_template_parameters(ptr) ?: return null), memScope)
    }
    inline fun hasAssociatedConstraints(): Boolean {
        return clang_TemplateDecl_has_associated_constraints(ptr)
    }
    inline fun getTemplatedDecl(): NamedDeclApi? {
        return NamedDecl((clang_TemplateDecl_get_templated_decl(ptr) ?: return null), memScope)
    }
    inline fun isTypeAlias(): Boolean {
        return clang_TemplateDecl_is_type_alias(ptr)
    }
    inline fun setTemplateParameters(TParams: TemplateParameterList?): Unit {
        return clang_TemplateDecl_set_template_parameters(ptr, TParams?.ptr)
    }
    inline fun init(NewTemplatedDecl: NamedDeclApi?): Unit {
        return clang_TemplateDecl_init(ptr, NewTemplatedDecl?.ptr)
    }
    companion object {
        val size: Int
            inline get() {
                return clang_TemplateDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_TemplateDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_TemplateDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_TemplateDecl_classof_kind(K.value)
        }
        fun MemScope.TemplateDecl_Holder(): TemplateDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return TemplateDecl(memory, this)
        }
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_TemplateDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_TemplateDecl_as_clang_Decl(ptr)!!, memScope)
    }
}

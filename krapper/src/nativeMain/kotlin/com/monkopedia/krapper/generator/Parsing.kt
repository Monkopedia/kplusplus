...
// THE FRONT-END (#184). The parse is IN-PROCESS: [CppParser] walks Clang's C++ AST (on
// kplusplus-generated libclang-cpp bindings) and constructs the parse-output WrappedTU
// directly, so the pipeline enters at the post-parse point — the pre-resolution rewrites below
// run on the tree, then ParsedResolver wraps it. Parse and codegen used to be two binaries
...
private const val BASE_TU_FILENAME = "parity_base.cc"

/**
 * Prepare a freshly parsed [tu] for resolution: run the pre-resolution rewrites, then wrap it
 * in a ParsedResolver. Shared by the base parse ([parseHeader]) and the per-instantiation
 * forcing parse ([parseForcingModel]). No cursor->element memo exists across the separately
 * parsed trees; identity between them is structural (type-string keys), which is all the
 * forcing flow consults (alreadyBoundKeys / tracker keys / dedup are string-keyed).
 */
private suspend fun prepareParsed(tu: WrappedTU): ParsedResolver {
 ...
 * `KrapperForce_*` header — which `#include`s [userHeaders] by the same path strings the
 * wrapper compile gets — parse it, then reorder its namespace-level children into the BASE
 * tree's first-seen order.
 *
 * On the libclang path the base parse and every forcing re-parse shared one Index, so
 * ModelFactories' USR-keyed elementLookup memo collapsed each namespace onto the element the
 * base parse had already populated — re-encountered decls kept their base position and only NEW
 * decls appended. A forcing TU's own lexical order DIFFERS from the base TU's whenever
 * ForcingHeader prepends a std include (e.g. `#include <vector>` pulls <new> in front of the
 * user header's <cwchar>), so a tree taken as-parsed would emit that round's std functions in a
 * SPEC-DEPENDENT order. First-seen base order is the spec-invariant choice.
 */
internal suspend fun parseForcingModel(
    target: String,
    userHeaders: List<String>,
    std: String
): ParsedResolver {

...
import com.monkopedia.krapper.parser.noncopyableDeterminismCheck
...
class Krapper : CliktCommand() {
    ...
    val includeDir by option(
        "--include-dir",
        help = "Extra include directory (-I), from the module's " +
            "`kplusplus { headerDirectory(...) }`. Repeatable. Threaded to BOTH the header " +
            "parse and the generated C++ wrapper's compile (where it is folded in alongside " +
            "the header files' own parent dirs), so cross-directory quote-includes resolve " +
    ...
    val dumpModel by option(
        "--dump-model",
        help = "Debug aid: write the parse-output model(s) this run consumed to <dir> as " +
            "ModelIo JSON — base_model.json for the header parse plus one KrapperForce_*.json " +
            "per --instantiate forcing parse. Off the hot path; the parse -> resolve handoff " +
            "is in-process."
    )
    val noncopyableDeterminism by option(
        "--noncopyable-determinism",
        help = "Run the #101 NonCopyable special-member determinism guard (parse a " +
            "self-contained v8::Persistent reduction repeatedly and assert the front-end's " +
            "special-member set never varies), then exit. Ignores all other options."
    ).flag()
    ...
    override fun run() {
        ...
        if (noncopyableDeterminism) {
            if (noncopyableDeterminismCheck() != 0) exitProcess(1)
            return
        }
        if (serviceMode) {
            return runService()

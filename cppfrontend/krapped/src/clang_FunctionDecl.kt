...
import clang.ConstexprSpecKind.Companion.fromValue as ConstexprSpecKindCompanionfromValue
...
import krapper.cppfrontend.internal.clang_FunctionDecl_align_of
import krapper.cppfrontend.internal.clang_FunctionDecl_as_clang_Decl
import krapper.cppfrontend.internal.clang_FunctionDecl_as_clang_DeclContext
import krapper.cppfrontend.internal.clang_FunctionDecl_as_clang_DeclaratorDecl
import krapper.cppfrontend.internal.clang_FunctionDecl_as_clang_NamedDecl
import krapper.cppfrontend.internal.clang_FunctionDecl_as_clang_ValueDecl
import krapper.cppfrontend.internal.clang_FunctionDecl_body_contains_immediate_escalating_expressions
import krapper.cppfrontend.internal.clang_FunctionDecl_cast_from_decl_context
import krapper.cppfrontend.internal.clang_FunctionDecl_cast_to_decl_context
import krapper.cppfrontend.internal.clang_FunctionDecl_classof
import krapper.cppfrontend.internal.clang_FunctionDecl_classof_kind
...
import krapper.cppfrontend.internal.clang_FunctionDecl_dyncast_clang_CXXConstructorDecl
import krapper.cppfrontend.internal.clang_FunctionDecl_dyncast_clang_CXXDestructorDecl
import krapper.cppfrontend.internal.clang_FunctionDecl_dyncast_clang_CXXMethodDecl
...
import krapper.cppfrontend.internal.clang_FunctionDecl_size_of
...
// WARNING: polymorphic class with a non-virtual destructor — deleting a FunctionDecl through a base pointer is undefined in C++ (the derived destructor will not run). Give the base a `virtual ~...()` to fix.

@krapper.CppBinding("clang::FunctionDecl")
class FunctionDecl(
    ...
    inline fun getConstexprKind(): ConstexprSpecKind {
        return ConstexprSpecKindCompanionfromValue(clang_FunctionDecl_get_constexpr_kind(ptr))
    }
    ...
    companion object {
        val size: Int
            inline get() {
                return clang_FunctionDecl_size_of()
            }

        val align: Int
            inline get() {
                return clang_FunctionDecl_align_of()
            }

        inline fun MemScope.classof(D: DeclApi?): Boolean {
            return clang_FunctionDecl_classof(D?.ptr)
        }
        inline fun MemScope.classofKind(K: Kind): Boolean {
            return clang_FunctionDecl_classof_kind(K.value)
        }
        inline fun MemScope.castToDeclContext(D: FunctionDeclApi?): DeclContext? {
            return DeclContext((clang_FunctionDecl_cast_to_decl_context(D?.ptr) ?: return null), memScope)
        }
        inline fun MemScope.castFromDeclContext(DC: DeclContext?): FunctionDeclApi? {
            return FunctionDecl((clang_FunctionDecl_cast_from_decl_context(DC?.ptr) ?: return null), memScope)
        }
        fun MemScope.FunctionDecl_Holder(): FunctionDecl {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return FunctionDecl(memory, this)
        }
    }
    inline fun asDeclaratorDecl(): DeclaratorDecl {
        return DeclaratorDecl(clang_FunctionDecl_as_clang_DeclaratorDecl(ptr)!!, memScope)
    }
    inline fun asDeclContext(): DeclContext {
        return DeclContext(clang_FunctionDecl_as_clang_DeclContext(ptr)!!, memScope)
    }
    inline fun asValueDecl(): ValueDecl {
        return ValueDecl(clang_FunctionDecl_as_clang_ValueDecl(ptr)!!, memScope)
    }
    inline fun asNamedDecl(): NamedDecl {
        return NamedDecl(clang_FunctionDecl_as_clang_NamedDecl(ptr)!!, memScope)
    }
    inline fun asDecl(): Decl {
        return Decl(clang_FunctionDecl_as_clang_Decl(ptr)!!, memScope)
    }
    inline fun asCXXMethodDecl(): CXXMethodDecl? {
        val raw: COpaquePointer = (clang_FunctionDecl_dyncast_clang_CXXMethodDecl(ptr) ?: return null)
        return CXXMethodDecl(raw, memScope)
    }
    inline fun asCXXConstructorDecl(): CXXConstructorDecl? {
        val raw: COpaquePointer = (clang_FunctionDecl_dyncast_clang_CXXConstructorDecl(ptr) ?: return null)
        return CXXConstructorDecl(raw, memScope)
    }
    inline fun asCXXDestructorDecl(): CXXDestructorDecl? {
        val raw: COpaquePointer = (clang_FunctionDecl_dyncast_clang_CXXDestructorDecl(ptr) ?: return null)

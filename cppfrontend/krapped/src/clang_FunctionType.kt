...
import clang.functionType.ArmStateValue
import clang.functionType.ArmStateValue.Companion.fromValue as ArmStateValueCompanionfromValue
...
import krapper.cppfrontend.internal.clang_FunctionType_align_of
import krapper.cppfrontend.internal.clang_FunctionType_as_clang_Type
import krapper.cppfrontend.internal.clang_FunctionType_classof
import krapper.cppfrontend.internal.clang_FunctionType_dyncast_clang_FunctionProtoType
import krapper.cppfrontend.internal.clang_FunctionType_get_arm_za_state
import krapper.cppfrontend.internal.clang_FunctionType_get_arm_zt0state
...
import krapper.cppfrontend.internal.clang_FunctionType_get_name_for_call_conv
...
import krapper.cppfrontend.internal.clang_FunctionType_size_of
...
// WARNING: polymorphic class with a non-virtual destructor — deleting a FunctionType through a base pointer is undefined in C++ (the derived destructor will not run). Give the base a `virtual ~...()` to fix.

@krapper.CppBinding("clang::FunctionType")
class FunctionType(
    ...
    inline fun getCallConv(): CallingConv {
        return CallingConvCompanionfromValue(clang_FunctionType_get_call_conv(ptr))
    }
    ...
    companion object {
        val size: Int
            inline get() {
                return clang_FunctionType_size_of()
            }

        val align: Int
            inline get() {
                return clang_FunctionType_align_of()
            }

        inline fun MemScope.getArmZAState(AttrBits: UInt): ArmStateValue {
            return ArmStateValueCompanionfromValue(clang_FunctionType_get_arm_za_state(AttrBits))
        }
        inline fun MemScope.getArmZT0State(AttrBits: UInt): ArmStateValue {
            return ArmStateValueCompanionfromValue(clang_FunctionType_get_arm_zt0state(AttrBits))
        }
        inline fun MemScope.classof(T: TypeApi?): Boolean {
            return clang_FunctionType_classof(T?.ptr)
        }
        inline fun MemScope.getNameForCallConv(CC: CallingConv): String? {
            val str: CPointer<ByteVar>? = clang_FunctionType_get_name_for_call_conv(CC.value)
            val ret: String? = str?.toKString()
            free(str)
            return ret
        }
        fun MemScope.FunctionType_Holder(): FunctionType {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return FunctionType(memory, this)
        }
    }
    inline fun asType(): Type {
        return Type(clang_FunctionType_as_clang_Type(ptr)!!, memScope)
    }
    inline fun asFunctionProtoType(): FunctionProtoType? {
        val raw: COpaquePointer = (clang_FunctionType_dyncast_clang_FunctionProtoType(ptr) ?: return null)
        return FunctionProtoType(raw, memScope)
    }
}

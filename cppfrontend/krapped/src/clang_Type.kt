...
import krapper.cppfrontend.internal.clang_Type_align_of
...
import krapper.cppfrontend.internal.clang_Type_dyncast_clang_ArrayType
import krapper.cppfrontend.internal.clang_Type_dyncast_clang_ConstantArrayType
import krapper.cppfrontend.internal.clang_Type_dyncast_clang_FunctionProtoType
import krapper.cppfrontend.internal.clang_Type_dyncast_clang_FunctionType
import krapper.cppfrontend.internal.clang_Type_dyncast_clang_TemplateTypeParmType
...
import krapper.cppfrontend.internal.clang_Type_size_of

// BEGIN KRAPPER GEN for clang::Type
// WARNING: polymorphic class with a non-virtual destructor — deleting a Type through a base pointer is undefined in C++ (the derived destructor will not run). Give the base a `virtual ~...()` to fix.

@krapper.CppBinding("clang::Type")
class Type(
    ...
    companion object {
        val size: Int
            inline get() {
                return clang_Type_size_of()
            }

        val align: Int
            inline get() {
                return clang_Type_align_of()
            }

        fun MemScope.Type_Holder(): Type {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return Type(memory, this)
        }
    }
    inline fun asArrayType(): ArrayType? {
        val raw: COpaquePointer = (clang_Type_dyncast_clang_ArrayType(ptr) ?: return null)
        return ArrayType(raw, memScope)
    }
    inline fun asFunctionType(): FunctionType? {
        val raw: COpaquePointer = (clang_Type_dyncast_clang_FunctionType(ptr) ?: return null)
        return FunctionType(raw, memScope)
    }
    inline fun asFunctionProtoType(): FunctionProtoType? {
        val raw: COpaquePointer = (clang_Type_dyncast_clang_FunctionProtoType(ptr) ?: return null)
        return FunctionProtoType(raw, memScope)
    }
    inline fun asConstantArrayType(): ConstantArrayType? {
        val raw: COpaquePointer = (clang_Type_dyncast_clang_ConstantArrayType(ptr) ?: return null)
        return ConstantArrayType(raw, memScope)
    }
    inline fun asTemplateTypeParmType(): TemplateTypeParmType? {
        val raw: COpaquePointer = (clang_Type_dyncast_clang_TemplateTypeParmType(ptr) ?: return null)
        return TemplateTypeParmType(raw, memScope)
    }
}

...
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_align_of
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_as_clang_Type
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_classof
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_desugar
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_get_decl
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_get_depth
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_get_index
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_is_parameter_pack
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_is_sugared
import krapper.cppfrontend.internal.clang_TemplateTypeParmType_size_of
...
@krapper.CppBinding("clang::TemplateTypeParmType")
class TemplateTypeParmType(
    ...
    inline fun getDepth(): UInt {
        return clang_TemplateTypeParmType_get_depth(ptr)
    }
    inline fun getIndex(): UInt {
        return clang_TemplateTypeParmType_get_index(ptr)
    }
    inline fun isParameterPack(): Boolean {
        return clang_TemplateTypeParmType_is_parameter_pack(ptr)
    }
    inline fun getDecl(): TemplateTypeParmDecl? {
        return TemplateTypeParmDecl((clang_TemplateTypeParmType_get_decl(ptr) ?: return null), memScope)
    }
    inline fun isSugared(): Boolean {
        return clang_TemplateTypeParmType_is_sugared(ptr)
    }
    inline fun desugar(): QualType {
        val retValue: QualType = memScope.QualType_Holder()
        clang_TemplateTypeParmType_desugar(ptr, retValue.ptr)
        return retValue
    }
    companion object {
        val size: Int
            inline get() {
                return clang_TemplateTypeParmType_size_of()
            }

        val align: Int
            inline get() {
                return clang_TemplateTypeParmType_align_of()
            }

        inline fun MemScope.classof(T: TypeApi?): Boolean {
            return clang_TemplateTypeParmType_classof(T?.ptr)
        }
        fun MemScope.TemplateTypeParmType_Holder(): TemplateTypeParmType {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return TemplateTypeParmType(memory, this)
        }
    }
    inline fun asType(): Type {
        return Type(clang_TemplateTypeParmType_as_clang_Type(ptr)!!, memScope)
    }
}

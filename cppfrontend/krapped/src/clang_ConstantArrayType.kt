...
import clang.ArraySizeModifier.Companion.fromValue
...
import krapper.cppfrontend.internal.clang_ArrayType_get_element_type
import krapper.cppfrontend.internal.clang_ArrayType_get_index_type_cvr_qualifiers
import krapper.cppfrontend.internal.clang_ArrayType_get_size_modifier
import krapper.cppfrontend.internal.clang_ConstantArrayType_align_of
import krapper.cppfrontend.internal.clang_ConstantArrayType_as_clang_ArrayType
import krapper.cppfrontend.internal.clang_ConstantArrayType_as_clang_Type
import krapper.cppfrontend.internal.clang_ConstantArrayType_classof
import krapper.cppfrontend.internal.clang_ConstantArrayType_desugar
import krapper.cppfrontend.internal.clang_ConstantArrayType_get_limited_size
import krapper.cppfrontend.internal.clang_ConstantArrayType_get_max_size_bits
import krapper.cppfrontend.internal.clang_ConstantArrayType_get_num_addressing_bits__const_clang_ASTContext_and
import krapper.cppfrontend.internal.clang_ConstantArrayType_get_s_ext_size
import krapper.cppfrontend.internal.clang_ConstantArrayType_get_size_bit_width
import krapper.cppfrontend.internal.clang_ConstantArrayType_get_z_ext_size
import krapper.cppfrontend.internal.clang_ConstantArrayType_is_sugared
import krapper.cppfrontend.internal.clang_ConstantArrayType_is_zero_size
import krapper.cppfrontend.internal.clang_ConstantArrayType_size_of
...
@krapper.CppBinding("clang::ConstantArrayType")
class ConstantArrayType(
    ...
    inline fun getSizeModifier(): ArraySizeModifier {
        return fromValue(clang_ArrayType_get_size_modifier(ptr))
    }
    ...
    inline fun getSizeBitWidth(): UInt {
        return clang_ConstantArrayType_get_size_bit_width(ptr)
    }
    inline fun isZeroSize(): Boolean {
        return clang_ConstantArrayType_is_zero_size(ptr)
    }
    inline fun getZExtSize(): ULong {
        return clang_ConstantArrayType_get_z_ext_size(ptr)
    }
    inline fun getSExtSize(): Long {
        return clang_ConstantArrayType_get_s_ext_size(ptr)
    }
    inline fun getLimitedSize(): ULong {
        return clang_ConstantArrayType_get_limited_size(ptr)
    }
    inline fun isSugared(): Boolean {
        return clang_ConstantArrayType_is_sugared(ptr)
    }
    inline fun desugar(): QualType {
        val retValue: QualType = memScope.QualType_Holder()
        clang_ConstantArrayType_desugar(ptr, retValue.ptr)
        return retValue
    }
    inline fun getNumAddressingBits(Context: ASTContext): UInt {
        return clang_ConstantArrayType_get_num_addressing_bits__const_clang_ASTContext_and(ptr, Context.ptr)
    }
    companion object {
        val size: Int
            inline get() {
                return clang_ConstantArrayType_size_of()
            }

        val align: Int
            inline get() {
                return clang_ConstantArrayType_align_of()
            }

        inline fun MemScope.getMaxSizeBits(Context: ASTContext): UInt {
            return clang_ConstantArrayType_get_max_size_bits(Context.ptr)
        }
        inline fun MemScope.classof(T: TypeApi?): Boolean {
            return clang_ConstantArrayType_classof(T?.ptr)
        }
        fun MemScope.ConstantArrayType_Holder(): ConstantArrayType {
            val memory: COpaquePointer = (interpretCPointer(alloc(size, align).rawPtr) ?: error("Allocation failed"))
            return ConstantArrayType(memory, this)
        }
    }
    inline fun asArrayType(): ArrayType {
        return ArrayType(clang_ConstantArrayType_as_clang_ArrayType(ptr)!!, memScope)
    }
    inline fun asType(): Type {
        return Type(clang_ConstantArrayType_as_clang_Type(ptr)!!, memScope)
    }
}

...
// plain user class `Widget`. The default ctor surfaces as `ms.Widget()` and zeroes
...
class UcConstructTest {
    ...
    @Test fun args_ctor_reflects_arguments() = memScoped {
        val ms = this
        val w = with(Widget) { ms.Widget__int_double(3, 1.5) }
    ...
    @Test fun both_ctors_usable_from_one_binding() = memScoped {
        ...
        with(Widget) {
            val def = ms.Widget()
            val args = ms.Widget__int_double(7, 2.25)

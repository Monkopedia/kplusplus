...
// this module runs the real v2 flow: the kplusplus compiler plugin drives the
// in-tree :krapper tool to wrap a C++ header into Kotlin bindings, and the tests
// exercise *those* — so this is the GENERATOR REGRESSION HARNESS, and it must bind
// against the sibling :krapper build, never the previous release's bundled tool
// (which is what the plugin's sibling-project resolution gives it). This is where
// std::-type rows (Group B in the matrix) get validated.
//
...
kplusplus {
    ...
    // them in — no separate impl library).
    ...
    // Build-seeded template instantiations. Their Kotlin facades are themselves generated, so
    // the compiler can't request them via SYNC_REQUIRED (see the instantiate() KDoc).
